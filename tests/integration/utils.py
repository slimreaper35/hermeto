...
def fetch_deps_and_check_output(
    ...
    if test_params.check_output:
        ...
        sbom = _normalize_timestamps(_load_json_or_yaml(output_dir.joinpath("bom.json")))
        ...
        expected_sbom = _normalize_timestamps(_load_json_or_yaml(expected_sbom_path))
...
def _normalize_timestamps(json_obj: Any, fixed_timestamp: str = "2025-01-01T00:00:00Z") -> Any:
    """
    Recursively replace all "timestamp" values with a fixed timestamp.
    This ensures deterministic test output by normalizing timestamps in the SBOM.
    """
    if isinstance(json_obj, dict):
        return {
            key: fixed_timestamp
            if key == "timestamp"
            else _normalize_timestamps(value, fixed_timestamp)
            for key, value in json_obj.items()
        }

    if isinstance(json_obj, list):
        return [_normalize_timestamps(item, fixed_timestamp) for item in json_obj]
