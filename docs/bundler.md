...
If your project includes git dependencies, Hermeto pre-fetches each repository
as a bare clone and redirects bundler's git fetches to the local copies using
git's
[`url.insteadOf`](https://git-scm.com/docs/git-config#Documentation/git-config.txt-urlltbasegtinsteadOf)
mechanism. The generated environment file will include `GIT_CONFIG_COUNT`,
`GIT_CONFIG_KEY_N`, and `GIT_CONFIG_VALUE_N` variables, for example:

```bash
export GIT_CONFIG_COUNT=2
export GIT_CONFIG_KEY_0=url.file:///tmp/hermeto-output/deps/bundler/myrepo-26487618a684/.insteadOf
export GIT_CONFIG_VALUE_0=https://github.com/org/myrepo
export GIT_CONFIG_KEY_1=protocol.file.allow
export GIT_CONFIG_VALUE_1=always
```

> [!WARNING]
> These variables are set globally for the git process. If your build
> environment already defines `GIT_CONFIG_*` variables (e.g. from an outer CI
> configuration), the values generated by Hermeto will overwrite them.
