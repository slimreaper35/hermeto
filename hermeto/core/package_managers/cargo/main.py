...
def _fetch_dependencies(package_dir: RootedPath, request: Request) -> dict[str, Any]:
    ...
    with _hidden_cargo_config_file(package_dir):
        # The necessary configuration to use the vendored sources will be printed to stdout after
...
def _resolve_main_package(package_dir: RootedPath) -> tuple[str, Optional[str]]:
    ...
    package_info: dict[str, Any] = parsed_toml.get("package", {})
    workspace_info: dict[str, Any] = parsed_toml.get("workspace", {})

    # fallback to the package directory name
    name = package_info.get("name", package_dir.path.stem)
    # version is optional field in the SBOM
    version = package_info.get("version", None)

    # check if the package is a workspace and has a version set in the workspace package table
    # https://doc.rust-lang.org/cargo/reference/workspaces.html#the-package-table
    if version == {"workspace": True}:
        version = workspace_info["package"]["version"]
...
def _resolve_cargo_package(package_dir: RootedPath) -> chain[Component]:
    ...
    packages = _parse_toml_project_file(package_dir.path / "Cargo.lock").get("package", [])
...
def _swap_sources_directory_for_subsitution_slot(template: str) -> dict[str, Any]:
...
def _use_vendored_sources(package_dir: RootedPath, config_template: dict[str, Any]) -> ProjectFile:
