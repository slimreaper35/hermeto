...
class PackageJson(UserDict):
    ...
    def from_dir(cls, path: Path) -> "PackageJson":
        """Create a PackageJson object from a package.json file."""
        return cls.from_file(path / PACKAGE_JSON)
...
@contextmanager
def _hidden_pnpmfiles(project_dir: Path) -> Generator[None, None, None]:
    """Temporarily hide pnpmfiles from the project directory."""
